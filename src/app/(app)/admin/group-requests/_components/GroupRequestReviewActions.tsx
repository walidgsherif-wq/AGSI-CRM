...
/**
 * Mirrors /admin/level-requests/_components/ReviewActions but calls
 * the company-group approve/reject server actions.
 */
export function GroupRequestReviewActions({
  requestId,
}: {
  requestId: string;
}) {
  ...
  function approve() {
    ...
    startTransition(async () => {
      const r = await approveGroupRequest(requestId, reviewNote.trim() || null);
  ...
  function reject() {
    ...
    setError(null);
    startTransition(async () => {
      const r = await rejectGroupRequest(requestId, reviewNote);
      if (r.error) setError(r.error);
      else {
        setReviewNote('');
        setRejectMode(false);
