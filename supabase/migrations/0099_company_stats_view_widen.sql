...
-- and every existing consumer's column stays exactly as before.
--
-- Safe re-run: CREATE OR REPLACE VIEW with the same primary shape;
...
SELECT
    c.id                                                              AS company_id,
    c.canonical_name,
    -- NEW columns so filters + membership scoping run on the view:
    c.company_type,
    c.city,
    c.is_active,
...
COMMENT ON VIEW company_stats IS
    'Per-company sortable/filterable stats: project_value_involved (FX-002 rule — shared projects count toward each linked company), project_count, est_steel_value, days_since_last_contact + engagement_bucket. Widened in 0099 with company_type / city / is_active / merged_into_company_id so downstream queries can drive from the view directly (ORDER BY value/count at the DB rather than sorting a name-first pre-fetch cap in memory).';
