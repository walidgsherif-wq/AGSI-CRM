-- 0061_f8_leadership_report_active_definition.sql
-- F8 (2/2) — align "active" in generate_leadership_report() to
-- is_active=true only (per the audit's F8 decision, option A).
--
-- 0036 had four filter pairs that diverged from the rest of the app:
--   :72   total_active_accounts headline used is_active AND has_active_projects
--   :214  level-distribution heat-map used is_active AND is_in_kpi_universe
--   :241  engagement-freshness heat-map used is_active AND is_in_kpi_universe
--   :253  geographic heat-map used is_active AND is_in_kpi_universe
--
-- After this migration, all four use is_active=true only — matching
-- the companies list, pipeline kanban, insights maps (PR #81 covers
-- those).
--
-- Old finalised reports are immutable (payload_json frozen at
-- generation per 0037) — they keep their original numbers. Only
-- reports REGENERATED or NEWLY CREATED after this migration use the
-- new definition.
--
-- Body copied verbatim from 0036_generate_leadership_report.sql; only
-- the four filter clauses changed. Idempotent via CREATE OR REPLACE.
    ...
    SELECT jsonb_build_object(
        'total_active_accounts', (
            SELECT COUNT(*) FROM companies
             WHERE is_active = true                                      -- F8: was AND has_active_projects=true
        ),
        'new_l3_this_period', (
            ...
            SELECT COALESCE(jsonb_object_agg(current_level::text, n), '{}'::jsonb)
              FROM (
                SELECT current_level, COUNT(*) AS n
                  FROM companies
                 WHERE is_active = true                                  -- F8: was AND is_in_kpi_universe=true
            ...
            SELECT jsonb_build_object(
              ...
              FROM (
                SELECT
                 ...
                 WHERE c.is_active = true                                -- F8: was AND c.is_in_kpi_universe=true
                 GROUP BY c.id
              ) t
        ),
        'geographic', (
